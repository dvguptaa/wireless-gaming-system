...
static cy_rslt_t bno055_write_reg(uint8_t reg, uint8_t data)
{
    ...
    task_print_info("BNO055: TX -> [ %02X %02X %02X %02X %02X ]", packet[0], packet[1], packet[2], packet[3], packet[4]);
...
static cy_rslt_t bno055_read_regs(uint8_t reg, uint8_t *buffer, uint8_t len)
{
    ...
    task_print_info("BNO055: TX(Read) -> %02X %02X %02X %02X", cmd[0], cmd[1], cmd[2], cmd[3]);
    ...
    uint8_t header = 0;
    {
        const TickType_t hdr_deadline = xTaskGetTickCount() + pdMS_TO_TICKS(500);
        ...
        while (xTaskGetTickCount() < hdr_deadline) {
            ...
            if (rh == CY_RSLT_SUCCESS) {
                ...
                task_print_info("BNO055: Skipped byte while waiting for header: 0x%02X", header);
                continue;
            }
        }
        if (!got_header) {
    ...
    if (bno_uart_mutex) xSemaphoreGive(bno_uart_mutex);
    task_print_info("BNO055: Read resp: header=0x%02X len=%u data=%02X", header, reported_len, buffer[0]);
    return CY_RSLT_SUCCESS;
}
